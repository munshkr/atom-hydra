...
export default class Main {
  ...
  defineLoadFunction() {
    window.load = function(moduleName, namespace) {
      const mod = require(`hydra-${moduleName}`);

      if (namespace) {
        window[namespace] = mod;
      } else {
        if (typeof mod === "object") {
          ...
          alert(
            `${moduleName} does not export an object.\n` +
              `You must provide a namespace (e.g. load("${moduleName}", "myMod"))`
          );
        }
      }
    };
  ...
  setGlobalDirPath() {
    let path = process.env["ATOM_NODE_PATH"];
    if (!path) {
      // If ATOM_NODE_PATH is not defined, run `npm root -g` command to get path
      const execSync = require("child_process").execSync;
      path = execSync("npm root -g --quiet")
        .toString()
        .trim();
      console.log(`global dir path from npm: ${JSON.stringify(path)}`);
    }

    if (!module.globalPaths.includes(path)) {
